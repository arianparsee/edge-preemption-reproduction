"""Run the configured Stage-10I official Pipeline DK-P hand example."""
...
CONFIG_PATH = Path("configs/stage10i_pipeline_dkp_example.json")
OUTPUT_PATH = Path("results/raw/stage10i/pipeline_dkp_example.json")
...
def run_example(config_path: Path = CONFIG_PATH) -> dict[str, object]:
    """Execute twice and compare the actual DK-P GA subset with Exact."""
    ...
    config = PipelineDKPConfig.from_workload(ga=_build_ga_config(raw), workload_tasks=tasks)
    policy = PipelineDoubleKnapsackPreemptionPolicy(config)
    first = policy.run(state, requesting_task_ids=requesting, time_remaining_by_task=times)
    second = policy.run(state, requesting_task_ids=requesting, time_remaining_by_task=times)

    exact_selected = ExactUtilityKnapsackSelector().select(
        capacity=server.capacity,
        tasks=tasks,
    )
    official_selected = first.round_two_knapsack_by_server[server.server_id]
    ...
    reproducible = (
        ...
        and first.retained_task_ids == second.retained_task_ids
        and first.preempted_task_ids == second.preempted_task_ids
        ...
        and dict(first.round_two_knapsack_by_server) == dict(second.round_two_knapsack_by_server)
        and dict(first.round_two_scores_by_server) == dict(second.round_two_scores_by_server)
    ...
    return {
        ...
        "round_two_knapsack_by_server": {
            key: list(value) for key, value in first.round_two_knapsack_by_server.items()
        },
        "round_two_scores_by_server": {
            ...
            for server_id, entries in first.round_two_scores_by_server.items()
        },
        "round_two_bid_count": len(first.round_two.bids),
        "round_two_price_status": first.metadata["round_two.price_status"],
        "accepted_task_ids": list(first.accepted_task_ids),
        "rejected_task_ids": list(first.rejected_task_ids),
        "retained_task_ids": list(first.retained_task_ids),
        "preempted_task_ids": list(first.preempted_task_ids),
        "final_task_states": {
        ...
        "exact_solver_auxiliary": {
            ...
            "selected_membership_equal": set(exact_selected) == set(official_selected),
        },
    }
