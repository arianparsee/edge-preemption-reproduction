...
پس از موفقیت Run `31700739166`، فایل `docs/stage15b_report.md` به مجموعه انتشار افزوده
شد. ممیزی نهایی فقط همین گزارش و به‌روزرسانی حاضر را پوشش می‌دهد؛ هیچ artifact، JSON
تشخیصی، داده خام، PDF یا metadata محلی به مخزن افزوده نمی‌شود. نتیجه ممیزی نهایی پیش از
commit باید `passed` باشد.
