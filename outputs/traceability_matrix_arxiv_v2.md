...
| STAGE15I-AGG | Stage 15-I؛ `[پیشنهاد فنی]` | رفع مسیر stale baseline و بازیابی aggregation-only بدون اجرای workload/policy | ۱۰۰ artifact موفق Run `32474360245`؛ reuse معتبر ۲۰ pair؛ baselineهای Stage 13-J | artifact نهایی تجمیع و checksum تحویل | حالت `aggregate-only` در `.github/workflows/stage15h-thirty-workload-repairs.yml`؛ workflow مستقل Stage 15-I برای ثبت آتی | cardinality 100+20؛ exact-one baseline path؛ finalizer 120/120؛ static no-execution gate | آماده dispatch روی شاخه بدون merge؛ هیچ فرض یا نتیجه علمی تغییر نمی‌کند |
