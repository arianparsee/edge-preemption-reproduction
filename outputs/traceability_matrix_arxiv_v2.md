...
| STAGE15H-AUX | Stage 15-H؛ خارج از روش مقاله | اعتبارسنجی ۳۰-workload دو repair تشخیصی feasibility برای DK-R/DK-P | baselineهای Stage 13-J/13-K؛ repairهای Stage 15-D.1/15-E؛ ۲۵ workload باقی‌مانده | ۱۲۰ repair pair، paired effects، CSV، نمودار و checksum | `scripts/run_stage15h_counterfactual.py`؛ `scripts/finalize_stage15h_validation.py` | 100/100 pair جدید؛ reuse 20/20؛ baseline 120/120؛ replay/RNG gate؛ checksum و completeness | کامل؛ Run `32474360245`؛ هر چهار repair/policy در 30/30 seed اثر Utility مثبت؛ `[آزمون کمکی]`؛ Figure 6 بدون تغییر |
| STAGE15I-AGG | Stage 15-I؛ `[پیشنهاد فنی]` | رفع مسیرهای stale/مفقود baseline و بازیابی aggregation-only بدون اجرای workload/policy | ۱۰۰ artifact موفق Run `32474360245`؛ reuse معتبر ۲۰ pair؛ ۱۲۰ baseline immutable از Run `31644121025` | artifact نهایی تجمیع و checksum تحویل | حالت `aggregate-only` در `.github/workflows/stage15h-thirty-workload-repairs.yml`؛ `scripts/materialize_stage15h_baseline_lifecycle.py`؛ workflow مستقل Stage 15-I برای ثبت آتی | cardinality 100+20؛ baseline 120/120؛ result/workload/policy seed manifest؛ lifecycle SHA `fac98f…4610a`؛ finalizer 120/120؛ static no-execution gate | Run `32829531291` عیب فنی lifecycle gitignored را آشکار کرد؛ اصلاح دوم محلی checksum-matched و آماده aggregation-only است؛ هیچ فرض یا نتیجه علمی تغییر نمی‌کند |
