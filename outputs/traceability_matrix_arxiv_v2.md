...
| STAGE15K2-AUX | Stage 15-K.2؛ خارج از روش مقاله | اعتبارسنجی محدود پنج-seed ASSUMP-049 و مشاهده aggregate تعامل آن با DK-P preemption | پنج seed نخست ASSUMP-033؛ seed اول reuse از Run 33663692202؛ baselineها و repair قبلی reuse-only | ۱۰ logical pair (۸ جدید، ۲ reuse)، paired CSV، aggregate report و checksum | `scripts/run_stage15k2_pair.py`؛ `scripts/finalize_stage15k2.py`؛ workflow Stage 15-K.2 | دو replay، RNG Option-A، capacity/state/Utility، Round-1 و PRE_ADMISSION ثابت، 10/10 completeness | تأییدشده فقط `[فرض آزمون کمکی]`؛ اجرای جدید محدود به ۴ seed × ۲ policy × ۲ replay؛ Figure 6 بدون تغییر |
| STAGE15M-PROP-054 | Stage 15-M.1؛ خارج از روش مقاله | ASSUMP-046 initial-population repair به‌همراه حفاظت دائمی batch-derived برای حذف cascade مستقیم Preemption | نخستین seed مرتب ASSUMP-033؛ فقط DK-P؛ baseline و repair-only checksum-pinned و reuse-only | یک logical pair، دو replay، aggregate guard/funnel/RNG report و checksum | `src/edge_reproduction/modified_methods/no_cascading_dkp.py`؛ `scripts/run_stage15m1_pilot.py`؛ workflow Stage 15-M.1 | fixture tests؛ capacity/state/Utility؛ protected-never-victim؛ replay/RNG Option-A؛ publication scan | Run `33872440661` معتبر ولی معیار Utility ناموفق؛ ردشده و غیرفعال؛ Pipeline رسمی و Figure 6 بدون تغییر |
| STAGE15M-PROP-055 | Stage 15-M.1B؛ خارج از روش مقاله | ASSUMP-046 به‌همراه cooldown فقط در نخستین Round-2 پس از activation؛ transaction دارای planned victim محافظت‌شده پس از selection به‌طور کامل abort می‌شود | seed `541501192080118187`؛ فقط DK-P؛ baseline، repair-only و ASSUMP-054 به‌صورت checksum-pinned و reuse-only | یک logical pair، دو replay، شمارنده cooldown/abort/admission suppression، Funnel/RNG و checksum | `src/edge_reproduction/modified_methods/one_auction_cooldown_dkp.py`؛ `scripts/run_stage15m1b_pilot.py`؛ workflow Stage 15-M.1B | fixture-only؛ replay/RNG Option-A؛ capacity/state/Utility؛ Round 1 و PRE_ADMISSION ثابت؛ publication scan | تأییدشده فقط `[فرض روش اصلاح‌شده پیشنهادی — آزمون کمکی]`؛ ASSUMP-054 غیرفعال؛ Pipeline رسمی و Figure 6 بدون تغییر |
