...
| STAGE15I-AGG | Stage 15-I؛ `[پیشنهاد فنی]` | رفع مسیر stale baseline و بازیابی aggregation-only بدون اجرای workload/policy | ۱۰۰ artifact موفق Run `32474360245`؛ reuse معتبر ۲۰ pair؛ baselineهای Stage 13-J | artifact نهایی تجمیع و checksum تحویل | `.github/workflows/stage15i-stage15h-aggregation-only.yml`؛ workflow اصلی Stage 15-H | cardinality 100+20؛ exact-one baseline path؛ finalizer 120/120؛ static no-execution gate | آماده dispatch؛ هیچ فرض یا نتیجه علمی تغییر نمی‌کند |
